...
\begin{itemize}[leftmargin=*]
    \item \textbf{Active states} involve the model performing operations beyond standard response generation: representing the truth while constructing a lie (deception), recognizing harmful content while declining to answer (refusal), or generating a response despite having its safety training removed (jailbreak). These states produce detectable geometric signatures because the additional processing creates measurable perturbations in cache structure.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Five detection capabilities demonstrated}: deception (AUROC $\approx 1.0$, 95\% CI $[0.85, 1.00]$), safety refusal ($\approx 0.90$, CI $[0.80, 1.00]$), jailbreak compliance ($\approx 0.88$), impossibility refusal ($\approx 0.95$, CI $[0.85, 1.00]$), and extended output suppression detection ($\approx 0.95$ with 9 features). Three of these---refusal, jailbreak, and impossibility detection---are new capabilities not previously reported. A sixth candidate---sycophancy detection---was refuted: the apparent signal (AUROC 0.9375) was a prompt-length confound, and the length-matched effect size ($d = 0.107$) is catastrophically underpowered.
    ...
    \item \textbf{Cross-model transfer solved}: logistic regression with standard scaling achieves AUROC 0.863 for cross-architecture deception detection, up from 0.67 with random forests, by exploiting the linear separability of aggregate cache features.
