...
\paragraph{Caveat: heterogeneous ``jailbreak'' class.} Abliteration does not produce uniform compliance. Manual inspection of the Qwen abliterated outputs shows approximately 8/20 genuinely providing harmful information, ${\sim}10/20$ warning but partially complying, and ${\sim}2/20$ hard-refusing despite abliteration. The classifier separates this heterogeneous mixture from benign responses, but the signal may reflect \emph{output suppression} (model withholding resources when processing harmful content) rather than jailbreak-specific geometry. This is consistent with the impossibility refusal finding (\Cref{sec:impossibility}): the signal is about withholding, not content type. A follow-up experiment (Experiment 41) tested abliterated models on \emph{benign} prompts, yielding AUROC $= 0.118$---below chance. This confirms that the classifier detects \emph{harmful-content processing}, not abliteration per se: when abliterated models process benign content, their cache geometry is indistinguishable from (or slightly more similar to) normal benign responses. The jailbreak signal is content-dependent, not a structural artifact of weight modification.
