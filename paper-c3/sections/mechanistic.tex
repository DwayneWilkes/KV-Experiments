...
\subsection{Per-Head Anatomy}
\label{sec:perhead}

Experiments 35 and 29 analyzed per-\emph{layer} structure; Experiment 49e extends this to per-\emph{head} resolution, ranking all 112 Qwen2.5-7B KV heads (28 layers $\times$ 4 KV heads per layer) by single-head AUROC for deception and refusal detection independently.

\paragraph{Deception: globally distributed.} Deception detection is \textbf{uniformly distributed} across all 112 heads. Single-head AUROCs range from 0.76 to 0.86, with peak performance at layer 12 (AUROC $\approx 0.85$). No head is ``dead'' ($< 0.60$), and no head dominates. Every head in the network carries a detectable deception signal, consistent with output suppression as a global processing mode rather than a localized circuit.

\paragraph{Refusal: concentrated in gatekeeper heads.} Refusal detection shows a strikingly different distribution. Two heads achieve AUROC $= 1.00$ (L4\_H1, L14\_H2), with several others at 0.98--0.99. However, multiple heads are at chance ($0.50$--$0.52$), contributing nothing to refusal detection. The refusal signal is \textbf{concentrated} in a small number of ``gatekeeper heads'' that appear to implement the suppression decision.

\paragraph{Correlation between conditions.} Head-level deception and refusal AUROCs are moderately correlated (Pearson $r = 0.381$, Spearman $\rho = 0.404$, $p < 0.0001$). The shared variance (${\sim}15\%$) suggests partial mechanistic overlap---the same heads that contribute to deception detection also tend to contribute to refusal detection, though refusal recruits specialized additional heads.

\paragraph{Interpretation.} The per-head anatomy reveals a mechanistic distinction between detection targets. Deception is a \emph{diffuse} cognitive state that perturbs every head in the network, consistent with the ``honest processing is richer'' framework (\Cref{sec:honest_richer}): output suppression reduces representational richness globally. Refusal, by contrast, involves \emph{concentrated} suppression circuitry---specific heads that implement the withholding decision---explaining why refusal detection transfers poorly across architectures (\Cref{tab:suppression_transfer}): the gatekeeper heads may occupy different positions in different model families.
...
The interpretation: honest response generation engages more of the model's representational capacity---more dimensions, more diverse key patterns, more heterogeneous processing. Output suppression---whether via deception, safety refusal, or impossibility refusal---constrains the model, reducing the representational richness of the output. This is consistent with the output suppression framework: the model withholds its full representational capacity when it is not producing unconstrained output, and this restraint is visible as sparser geometry at every layer of the transformer stack.
